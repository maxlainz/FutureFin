...
pub struct ProjectionSeriesResponse {
    ...
    ///
    /// **Qué mide cada nodo (decisión C9 del owner, 2026-09-07): «lo que necesitas TENER (líquido)
    /// a esa edad para jubilarte entonces al umbral».** El solver fija la acumulación hasta el mes
    /// anterior en la línea determinista y sortea solo desde ahí, así que todos los caminos llegan
    /// al nodo con esta misma cifra: es el capital con el que hay que llegar, no el percentil de tu
    /// cartera de hoy proyectada hasta esa edad. Antes de C9 era lo segundo, y por eso la curva
    /// crecía sin techo con el horizonte (medido en P9: de 2,9 M€ a 93,5 M€ en euros de hoy entre
    /// el mes 1 y el 840 — la incertidumbre de treinta años de acumulación, no una necesidad).
    ///
    ...
    /// mes equivocado. Quien la quiera «en dinero de hoy» deflacta la curva y la línea a la vez con
    /// el MISMO factor (`deflation_annual_inflation_percent`). Ojo con la asimetría respecto a
...
pub(crate) struct SimKpis {
    ...
    ///
    /// **F2 y F3 solo pueden firmar en el mes de jubilación**; a partir de ahí el único motivo
    /// vivo es F1.
