...
use futurefin_engine::{project_net_worth_series, ProjectionInput, SimInput};
...
// (d), (e) y (f) **se retiraron en E4**: medían la tabla del objetivo PUENTE (`P = 240` y
// `P = 840` gross-ups precomputados) y la bisección de `required_contribution_monthly`. El modelo
// v2 retiró las tres cosas del motor (M4): el objetivo ya no tiene base puente ni descuento, y los
// dos solves que biseccionaban contra él viven ahora en `crates/engine-stochastic`, donde el coste
// que hay que medir es el del SORTEO (`crates/engine-stochastic/tests/timing_mc.rs`), no el de una
