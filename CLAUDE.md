...
- `handlers/transactions/` — per-user **histórico de gasto mensual** under `/v1/transactions` (import CSV MyInvestor/N26, movimientos manuales, reglas de categorización, comparativa mes vs budget vs promedio). Modules: `crud.rs`, `import.rs` (preview→confirm stateless, presets en `csv_presets.rs`), `rules.rs`, `summary.rs`, `schema.rs`. Como los snapshots, las transacciones NO son inputs del engine → **ningún handler llama a `refresh_projection_after_mutation`** (regresión: `transactions_projection_cache.rs`).
...
**Transactions (histórico de gasto mensual, v1.6.0)**: cada usuario importa CSV bancarios (MyInvestor/N26, autodetección + dedup por huella canónica) o mete efectivo a mano, los categoriza con reglas aprendidas al confirmar un import, y ve la comparativa mes real vs presupuesto vs promedio (`/v1/transactions/*`). Per-user (`owner_user_id`; lecturas con `?view=mine`). Como los snapshots, las transacciones **no son inputs del engine** → sus mutaciones **nunca invalidan la cache de proyección** (regresión: `transactions_projection_cache.rs`). Tres tablas (`transaction_imports`, `transactions`, `categorization_rules`) incluidas en `.ffbackup` v5.
