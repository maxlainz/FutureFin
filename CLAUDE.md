...
**Servidor MCP (v3.0.0)**: endpoint `/mcp` (Streamable HTTP, SDK oficial `rmcp`) en el mismo binario y puerto, **solo lectura**. Dos credenciales, despachadas por prefijo del Bearer: **tokens de API** por usuario (`ffp_…`, Ajustes → Acceso) y **access tokens OAuth** (`ffo_…`, v3.1.0). Ninguna congela nada — cada request re-resuelve membership y rol (revocar = corte inmediato, misma filosofía que las sesiones en DB). Las tools reutilizan las core fns de los handlers (`summary_core`, `projection_series_cached`, …) — nunca dupliques SQL ni tipos de respuesta en una tool. `/mcp` no va en OpenAPI (se autodescribe vía `tools/list`). Detalles: [`.claude/api-routes.md`](.claude/api-routes.md) sección MCP.
