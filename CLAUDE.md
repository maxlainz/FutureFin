...
- `mcp/` — servidor MCP embebido (`/mcp`, Streamable HTTP, rmcp 3.1): **52 tools de lectura, simulación y escritura** que llaman a las mismas core fns `*_core` que los handlers HTTP (cero deriva, Decimal-as-string intacto; la invalidación de cache vive dentro de las cores de mutación). Auth = middleware Bearer (`mcp/auth.rs`) con identidad y rol vivos por request; toda tool de escritura pasa por `require_mcp_write` (rol + toggle `installation.mcp_write_enabled`, editable en Ajustes → Integraciones) y las destructivas piden `confirm: true` (sin él devuelven un preview). `FUTUREFIN_MCP_ENABLED=0` lo desmonta
