...
- `mcp/` — servidor MCP embebido (`/mcp`, Streamable HTTP, rmcp 3.1): 10 tools de **solo lectura** que llaman a las mismas core fns `*_core` que los handlers HTTP (cero deriva, Decimal-as-string intacto). Auth = middleware Bearer (`mcp/auth.rs`) que resuelve identidad y rol vivos por request. `FUTUREFIN_MCP_ENABLED=0` lo desmonta
...
**Servidor MCP (v3.0.0)**: endpoint `/mcp` (Streamable HTTP, SDK oficial `rmcp`) en el mismo binario y puerto, **solo lectura**. Auth por **tokens de API** por usuario (`ffp_…`, gestionados en Ajustes → Acceso): el token no congela nada — cada request re-resuelve membership y rol (revocar = corte inmediato, misma filosofía que las sesiones en DB). Las tools reutilizan las core fns de los handlers (`summary_core`, `projection_series_cached`, …) — nunca dupliques SQL ni tipos de respuesta en una tool. `/mcp` no va en OpenAPI (se autodescribe vía `tools/list`). Limitación documentada: el conector de claude.ai exige OAuth (fuera de scope). Detalles: [`.claude/api-routes.md`](.claude/api-routes.md) sección MCP.
