...
- **Campos inmutables en importadas**: en una fila con `import_id NOT NULL`, `op_date`/`amount`/`concept` son **inmutables** por PATCH (protegen la huella) → 400 `immutable_field`; para cambiarlos hay que borrar y re-añadir a mano. En manuales sí se recomputa la huella al cambiarlos.
...
- **`last_materialized_month` = cursor monotónico de idempotencia** (siempre el día 1 de un mes): el mes más reciente ya materializado. Materializar avanza mes a mes desde ahí hasta el mes civil actual y **nunca retrocede**. Al crear la regla desde un alta manual se fija al primer día del mes del alta, de modo que la instancia de origen no se re-materializa. **No hay `UNIQUE (rule, mes)` a propósito**: si el usuario borra una instancia materializada, re-ejecutar materialize **no** debe recrearla → el cursor es la única fuente de verdad de idempotencia (una constraint reintroduciría el movimiento borrado).
