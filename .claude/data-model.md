...
- `users`: `id (uuid PK)`, `username (unique)` (restricción `users_username_key`), `password_hash (text, NULLABLE)`, `birth_date (date nullable)`, `created_at`, `external_user_id (uuid nullable)`, `retirement_profile (jsonb nullable — 5.0.0)`. `password_hash` nullable y `external_user_id` llegan con `20260827120000_users_trusted_header_identity.sql` (SSO por cabeceras de un proxy de confianza), ambos **aditivos** para las cuentas que ya existían:
  - **`password_hash` deja de ser `NOT NULL`**. `NULL` = cuenta SSO **sin contraseña**: la autenticación la hizo el proveedor antes de que la petición llegara. Inventarle una contraseña aleatoria sería peor (una credencial que nadie conoce y que el cambio de contraseña podría rotar), así que la ausencia se modela como ausencia. Los **dos** flujos de contraseña que quedan la rechazan con **401 `sso_account_no_password`**: `POST /v1/auth/login` y `POST /v1/auth/password`. `POST /v1/backup/user-export` era el tercero y salió de la lista en 5.0.0 (issue #213): el `.ffbackup` lleva ahora su propia contraseña, que solo alimenta el KDF.
  ...
  - **`retirement_profile` (5.0.0, `20260902200000_users_retirement_profile.sql`, D13)**: el PLAN DE JUBILACIÓN de esa persona. `NULL` = defaults (y una clave ausente dentro del JSONB también: el struct se deserializa contra un gemelo con `#[serde(default)]`), así que es aditivo: nadie ve moverse un número. Forma completa y cotas en [`api-routes.md`](api-routes.md) §Perfil de jubilación por usuario; handler `handlers/retirement_profile.rs`. La migración **copia** al perfil de cada usuario los cuatro ejes que hasta 4.15.x vivían en `installation.fire_settings` (`fire_number_mode`, `fire_number_manual_amount`, `swr_pct`, `horizon_lifespan_age`) y **los retira** del JSONB de la instalación. Es input del motor: toda escritura invalida la cache de proyección.
    - **Forma v2 (5.0.0, decisiones M2/M4/M5/M6/C3/C7)**: `{strategy, target_retirement_age?, fire_number_mode, fire_number_manual_amount?, swr_pct, horizon_lifespan_age, success_threshold_pct, coast_mode, coast_stop_age?, withdrawal_rule, pension?, partial_retirement?}`. Dentro, `pension` lleva el PUENTE (`bridge_enabled`, `bridge_max_pct?`, `bridge_max_years?` — C7: el puente es un ajuste de la pensión, no una estrategia) y `partial_retirement` lleva `mode` (`at_age` | `asap`) con `starts_at_age` ya **opcional**.
    - **Cuatro claves retiradas del JSONB**: `success_threshold_pct` cuando era «se acepta y se ignora» (V7), `target_basis`, `bridge_discount_basis` y `cash_buffer_months`. Las borra `20260906091500_drop_stored_success_threshold.sql` (idempotente, sin DDL). **La lectura nunca dependió de esa migración**: `RetirementProfile` no lleva `deny_unknown_fields`, así que un perfil que las conserve carga igual y las ignora. El umbral se borra por una razón distinta de las otras tres: en v2 vuelve a ser load-bearing con default 95, y un 95 almacenado que nadie eligió se leería como elección — congelada el día que el default se mueva.
    - **`strategy: "pension_bridge"` sigue almacenado en perfiles viejos** y se lee como `asap` con el puente encendido (alias de compatibilidad, C7). El literal no se re-emite: la primera escritura deja `"strategy":"asap"` y materializa `pension.bridge_enabled: true`.
...
- `installation`: `id (uuid PK)`, `base_currency (char 3)`, `calendar_tz (text)`, `annual_inflation_assumption_percent (decimal NOT NULL, DEFAULT 2.5 desde 4.9.0/#146 — migración `20260901130000`, solo filas NUEVAS; rango [−2, 50] en la validación de escritura; indexa el GASTO del bucle Y el target FIRE — #139; negativo = deflación: ambos decrecen)`, `show_age_mode (text: 'dates'|'ages')`, `fire_settings (jsonb nullable)`, `mcp_write_enabled (bool NOT NULL DEFAULT TRUE, `20260818120000` — kill-switch vivo de las tools de escritura MCP; toggle owner-only en Ajustes → Integraciones; NO se exporta en el `.ffbackup`)`, `created_at`
  - `projection_target_age` fue **eliminada** (`20260516120000_drop_projection_target_age.sql`, v1.0.6): la edad de jubilación dejó de ser un ajuste **de la instalación**. **Desde 5.0.0 la fecha de jubilación es por USUARIO** (`users.retirement_profile.strategy`): con `retire_at_age` —y con `coast` en modo `fixed_retirement_age`— manda la edad (`target_retirement_age`); en el resto la decide el umbral de éxito (`success_threshold_pct`), con **un solo trigger por simulación**. Lo que sigue prohibido es una edad de jubilación en `installation`.
...
**persons** (viva, pero casi muerta): tabla demográfica anterior al modelo multiusuario. **Se sigue
LEYENDO en un solo sitio y para una sola cosa**: `resolve_projection_context`
(`apps/api/src/handlers/projection.rs:2738`) toma la primera `birth_date` no nula ordenando
`is_primary DESC, sort_index ASC` como **fallback demográfico** cuando el usuario de la sesión no
tiene la suya — y ese valor alimenta **únicamente** el horizonte derivado y el `viewer_birth_date`
publicado. **No puede alcanzar el trigger por edad, ni la pensión con fecha, ni la fase parcial**:
esos leen `ctx.session_birth_date`, sin fallback, porque heredar la fecha de otra persona para
decidir cuándo se jubila quien pregunta sería inventarle una edad (sin ella, las estrategias por
edad degradan a `asap` con `warnings: ["birth_date_missing"]`). En `?view=household` no se toca:
cada fecha sale de `users`. No hay migración de retirada y no se expone en el API
(`/v1/changes` la excluye a propósito).

**categories**: `id`, `installation_id`, `scope ('asset'|'liability'|'income'|'expense')`, `name`, `sort_index`, `is_fallback (bool NOT NULL DEFAULT false — 4.15.0)`. **Categoría por defecto**: exactamente una por `(installation_id, scope)` para `income` y `expense` (índice único parcial `categories_one_fallback_per_scope_idx … WHERE is_fallback`; CHECK `categories_fallback_scope_check`: solo esos dos scopes). Es el destino de todo movimiento/plantilla de esa clase que llega sin categoría; **no se borra** (`category_is_fallback`) — se cambia designando otra con `PATCH {is_fallback: true}` (swap atómico; `false` se rechaza). La siembra de instalación (`seed_default_categories`) marca «Otros gastos»/«Otros ingresos»; la migración `20260902120000_categories_fallback_and_transaction_category_required` las adoptó por nombre o las creó en las instalaciones existentes.
...
**budget_entries**: `id`, `installation_id`, `owner_user_id`, `category_id`, `scope ('income'|'expense')`, `amount (decimal, monthly)`, `notes`, `sort_index`, `persists_after_retirement (bool, default false)` — income entries only: whether this income continues after the **mes efectivo de jubilación** (el cruce FIRE con `asap`/`pension_bridge`, la edad objetivo con `retire_at_age`/`coast`; hasta 4.15.x solo existía el cruce) (used to compute `income_retirement_monthly` in projection) — plus `ends_at_retirement (bool, default false)` and `expense_end_date (date nullable)` — expense entries only: stop computing the expense at the FIRE crossover or at a fixed date (`20260514120000_budget_entries_add_expense_end.sql`, v1.0.8)

**planning_flows**: `id`, `installation_id`, `owner_user_id`, `category_id`, `title`, `expected_amount (decimal, CHECK > 0)`, `amount_basis (text NOT NULL DEFAULT 'one_off', CHECK IN ('one_off','per_month') — 4.11.0/#148)`, `due_date (date nullable; solo one_off)`, `window_start_date (date nullable)`, `window_end_date (date nullable)`, `show_in_chart (bool — `20260517120000_planning_flows_show_in_chart.sql`; solo puede ser true con one_off + due_date)`, `notes`, `sort_index`, `created_at`, `updated_at`. **No hay columna `direction`**: la dirección se DERIVA de `categories.scope` de su categoría (`income` → inflow, `expense` → outflow; `apps/api/src/handlers/planning.rs::scope_to_direction`) — una versión anterior de esta fila la inventaba (errata C6 del spike de la Ola 7). `amount_basis` declara la unidad del importe (`one_off` = TOTAL en €; `per_month` = **€/MES** durante `[window_start_date, window_end_date]`, `window_end_date` NULL = sin fin). Forma por base: CHECK `planning_flows_basis_shape_chk` (one_off sin ventana; per_month sin due_date y con window_start) + `planning_flows_window_order_chk` (end ≥ start) — migración `20260901140000_planning_flows_recurring_window.sql`.
...
- **…y por eso la lectura no se fía de él a ciegas (5.0.0).** «UUID generado por el servidor» significa que dos fotos de la misma cuenta pueden llegar con claves distintas: `POST /v1/history/snapshots` genera una nueva cuando el cuerpo no trae `item_id`, y la tool MCP `create_snapshot` **ni siquiera expone el campo**. Cada clave era un timeline propio, y el LOCF de #130 los apilaba: con 4 fotos, el `assets_total` del histórico se multiplicaba por 2 y por 3 en tramos sucesivos y caía a 0 tras la última. `GET /v1/history/series` resuelve ahora la identidad **al leer** (`resolve_item_identity`, `handlers/history.rs`): agrupa por etiqueta dentro del `(owner, kind)` y canoniza al id de la fila viva homónima cuando es inequívoca; la ambigüedad no fusiona nada. La columna y su UNIQUE **no cambian** — el arreglo es de lectura, y el camino que sí escribe una clave estable (`…/snapshots/prefill` → `item_id` reenviado por la SPA) sigue siendo el bueno. Detalle: `.claude/api-routes.md` §History series.
...
- **`kind='savings'` no lleva categoría** (`category_id` debe ser NULL → `savings_no_category`, validado en Rust); **`expense`/`income` la llevan SIEMPRE** (4.15.0): con categoría, el `scope` debe coincidir con el kind (`category_scope_mismatch`); sin ella, el alta/edición/lote/MCP resuelven a la categoría por defecto del scope (`resolve_category_for_kind`) y el confirm del import rechaza (`category_required`). Invariante: `category_id IS NULL ⇒ kind IS NULL OR kind = 'savings'`. Una fila con `kind` NULL es «sin clasificar», no «gasto sin categoría», y queda fuera del CHECK a propósito.
...
protocolo). Ninguna de aquellas cambió `CURRENT_SCHEMA_VERSION` (iba en **10** entonces; hoy es **13** — `grep -n 'CURRENT_SCHEMA_VERSION' apps/api/src/handlers/backup_user/schema.rs`). Contexto de diseño completo
...
- **Solo 8 de las 18 tools con preview lo exigen** (`grep -c '= two_phase(' apps/api/src/mcp/server.rs` frente a `grep -c 'p\.confirm\.unwrap_or(false)' …`; eran 8 de 17 en 4.15.x y 7 de 14 hasta la Fase 6) — las de cascada de
...
- `swr_pct` bounded **0–6** (percent, not ratio) — desde 5.0.0 vive en `users.retirement_profile`, y su techo subió de 4 a 6 con el modelo v2 (M5: el SWR es «el máximo que se vende en cualquier año», no el divisor de un objetivo). Constante única: `MAX_SWR_PCT` en `handlers/retirement_profile.rs`
