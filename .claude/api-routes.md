...
- **Base de gasto derivada/total (reforma 3.4.0)**: en B/C con datos, `expense_derived_monthly_equivalent` = **0** (no hay componente derivada: la cuota es un gasto más dentro del promedio; los pasivos solo restan su principal en `net_worth` y en la proyección) y `expense_total_monthly_equivalent` = `expense_avg`. Las dos identidades valen en los **tres** modos:
...
- `savings_expected_monthly_equivalent` (Decimal-string) — el **neto del presupuesto** (`net_monthly_equivalent` del snapshot de budget, cuotas derivadas incluidas), capturado **antes** del override B/C → en B/C puede diferir del `net_monthly_equivalent` servido.
- `savings_actual_monthly_avg_12m` (Decimal-string, opcional, `skip_serializing_if`) — promedio **bruto** `income_avg − expense_avg` de `transactions_12m_avg` (12 meses civiles completos, meses solo-recurrentes excluidos). Las cuotas pagadas cuentan como gasto — simétrico al esperado, que incluye las cuotas derivadas del presupuesto. Desde la reforma 3.4.0, en modo B este «real» coincide con `net_monthly_equivalent` (misma base cruda). Se **omite** del JSON cuando no hay meses con datos (ausente ≠ «ahorras 0 €»).
