...
- `savings_actual_monthly_avg_12m` (Decimal-string, opcional, `skip_serializing_if`) — promedio **bruto** `income_avg − expense_avg` de `transactions_12m_avg` (12 meses civiles completos, meses solo-recurrentes excluidos). **SIN resta híbrida de cuotas** — deliberado: las cuotas pagadas ya cuentan como gasto en los movimientos, simétrico al esperado que incluye las cuotas derivadas; por eso en modo B este «real» ≠ `net_monthly_equivalent` (que sí aplica la resta híbrida). Se **omite** del JSON cuando no hay meses con datos (ausente ≠ «ahorras 0 €»).
