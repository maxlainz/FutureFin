...
**`PATCH` de `fire_settings`: tri-estado real desde 4.4.2 (issue #95)** — omitir la clave conserva el JSON almacenado, `null` lo **borra** (la columna vuelve a `NULL` y `resolve_fire_settings` devuelve los defaults en la siguiente lectura: SWR 3,5 %, modo `annual_expense`, tramos ES), un objeto lo sustituye. `{"fire_settings": null}` como único campo es un PATCH válido. Antes de 4.4.2 el tipo ya era `Option<Option<FireSettings>>` pero le faltaba el `deserialize_with`, así que serde colapsaba el `null` presente con la clave ausente: la rama `Some(None) => None` del handler era **código muerto** y ese cuerpo salía por el 400 `patch_empty` — la respuesta decía «no has mandado ningún campo» al único cuerpo que el doc-comment (publicado en OpenAPI) documentaba para borrar. Arreglo: `handlers::deserialize_double_option`. **Borrar no es leer `null`**: como `resolve_fire_settings` rellena los defaults, la única forma de distinguir «columna a `NULL`» de «hay guardada una copia de los defaults» es mirar la columna — lo hace `installation_patch.rs::patch_fire_settings_is_a_real_tristate`. La tool MCP `update_fire_settings` no expone el borrado (mergea campo a campo sobre lo resuelto); su superficie no cambia.
...
**`PATCH` de `purchase_price`: tri-estado real desde 4.4.2 (issue #95)** — omitir la clave conserva, `null` **borra** (el activo vuelve a «sin coste declarado» y `unrealized_pnl` pasa a `null` con `no_purchase_price`), un valor sustituye. Un PATCH cuyo **único** contenido es `{"purchase_price": null}` es válido: `Some(None)` cuenta como campo presente en el guard `patch_empty`. Antes de 4.4.2 la rama de borrado existía en `merge_optional_decimal_patch` y era **inalcanzable**: la implementación por defecto de `Option<T>` en serde colapsa el `null` presente con la clave ausente, así que ese cuerpo salía por el 400 `patch_empty` y —peor— un PATCH de formulario completo con `purchase_price: null` devolvía 200 **sin borrar nada**, que es exactamente lo que manda la SPA al vaciar el campo (`App.tsx`, «PATCH: siempre enviar precio de compra»). El arreglo es el tipo, no el `match`: `Option<Option<serde_json::Value>>` + `handlers::deserialize_double_option`. El schema OpenAPI del campo pasa de `Object` a `String` en el mismo cambio — declaraba un tipo que el runtime nunca aceptó. La tool MCP conserva su flag `clear_purchase_price` (el tri-estado no es expresable en JSON Schema — doctrina de la Fase 2). Tests: `assets_unrealized_pnl.rs::patch_purchase_price_is_a_real_tristate`.
...
**`debt_service` es nullable desde 4.3.1** (issue #82, **breaking**: antes era siempre string). En los
modos B/C con base de gasto real la cuota ya es un movimiento dentro del promedio, así que contarla
aparte la duplicaría y el campo salía `"0"` — que un cliente lee como «no pagas servicio de deuda»
teniendo un préstamo vivo. Ahora sale `null` + `debt_service_absent_reason:
"included_in_real_expense"`; un `0` vuelve a significar solo «no hay pasivos con cuota activa». El
gate es **`expense_from_avg`** (de dónde salió la base de GASTO), no el modo: el fallback del promedio
es por lado, así que un modo B con datos de ingreso y sin datos de gasto publica
`savings_source: "transactions_avg"` y sin embargo **sí** cobra la cuota. La razón la decide un único
punto, `BuiltProjection::debt_service_absent_reason`, y la consumen las dos superficies que publican
la cifra (esta y `simulate_projection`). `recurring_net` sigue cerrando: con `debt_service` ausente el
sustraendo es 0. Pin: `projection_number_semantics.rs`.
...
- **`upcoming_flows_count`** (`i64`) y **`upcoming_last_due_date_ymd`** (`YYYY-MM-DD` u omitido) — los `upcoming_*_total` suman **TODOS** los `planning_flows` del scope, con y sin `due_date`, sin ventana temporal y sin anualizar: mezclaban un pago del mes que viene con uno de 2042 y las dos lecturas eran indistinguibles. El recuento hace que `0` signifique «no hay flujos» y no «se anulan»; la fecha máxima declara el horizonte que los totales no tienen. Salen del **mismo `GROUP BY`** que ya existía (dos columnas más, ninguna query nueva). Solo cuentan los scopes `income` y `expense`.
