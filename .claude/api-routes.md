...
- **Estrategia y fases (5.0.0, §B.8/§C)** — la jubilación deja de ser «un cruce» y pasa a ser lo que decide la ESTRATEGIA del perfil del usuario (`users.retirement_profile.strategy`):
  ...
  - `pension_start_month_index`, `partial_retirement_month_index` — rejilla 0-based (`engine_month_to_grid`): primer mes con pensión con fecha / primer mes de media jornada; `null` si el perfil no declara pensión con fecha o fase parcial (la pensión SIN fecha sigue viajando dentro del ingreso de jubilación y no tiene mes propio).
