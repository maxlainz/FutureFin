...
- Bash con git: leer/editar el repo, commitear, **empujar ramas** (probado). `cargo update` llega a crates.io (probado). `npm` llega al registry (probado en el estreno). **ANTES de tu primer commit, SIEMPRE**: `git config user.name "Max Lainz" && git config user.email "49809588+maxlainz@users.noreply.github.com"` — tu entorno comitea por defecto como `Claude <noreply@anthropic.com>`, y aunque el mensaje vaya limpio, GitHub añade el `Co-authored-by:` de ese autor al squashear tu PR, colando la atribución en main DESPUÉS de los gates (incidente #194, 2026-09-01; el scan de CI ahora también revisa identidades y pondrá tu PR en rojo).
