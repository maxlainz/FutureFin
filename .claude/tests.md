...
- `runway.rs` `mod tests`: 8 tests (v2.2.0) covering `liquid_runway_months` — exact reduction to `A/g` with no return/inflation (no tolerances), return extends / inflation shortens, `Indefinite` at the 100-year cap, value-weighted multiplier (vs the naive per-asset mean), negative return = zero growth, `NoExpenseBase` with zero expense, `Months(0)` with zero balance. Predicted values in each test's doc comment.
...
| `summary_runway.rs` (7) | `financial_health` de `GET /v1/summary`: **captura de regresión previa a 2.2.0** que sigue verde después (`runway_months == liquid_assets_total / expense_total` exacto en modo A sin retorno ni inflación; `expense_derived` = cuotas activas; sin gasto el campo `runway_months` **no se serializa**), más el comportamiento nuevo — la rentabilidad alarga (12.000 al 5 % con gasto 1.200 → >10 y <11), la inflación acorta (3 % → <10 y >9), `runway_is_indefinite` (1M al 7 % vs 1.000 €/mes → `runway_months` null + flag true), modo B con las identidades `expense_total = expense_reg + expense_der` y `net = income − expense_total` y runway sobre la base efectiva (16.000/1.600 = 10; en modo A serían 16.000/8.800), y el fallback `months_with_data == 0` → bloque idéntico al de modo A. Números predichos en el comentario de cada test. |
...
- Pure-function tests only. 293 en 11 ficheros a 2026-08-14 (esta lista omite `responsive.test.ts` y `chart-gestures.test.ts`; el total autoritativo lo da `npm test --workspace futurefin-web 2>&1 | grep Tests`):
  - `lib/format.test.ts` (33) — Intl formatting in es-ES, edge cases (null/NaN/empty), Decimal string preservation; `formatMonthsRough` en años + meses desde 24 y `formatRunwayValue` («Cubierto» si el runway es indefinido) (v2.2.0).
