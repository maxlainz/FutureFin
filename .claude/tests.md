...
- Pure-function tests only. **396 en 17 ficheros a 2026-08-24** (medido con el runner: `npm test --workspace futurefin-web 2>&1 | grep Tests`; eran 368 en 16 el 2026-08-22 y 321 en 13 el 2026-08-19). Los números entre paréntesis de la lista de abajo son **`it(` del fichero**, no tests ejecutados, y no cuadran con el total a propósito: `chart-gestures.test.ts` y `fire.test.ts` generan tests en bucles (10 `it(` → 77 y 2 → 8). Para un total, el runner; para orientarte, `grep -c 'it(' <fichero>`.
