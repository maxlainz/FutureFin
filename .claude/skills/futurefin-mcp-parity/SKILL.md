...
| El **tri-estado `null` = borrar** de los cuerpos PATCH (`PatchAssetBody.purchase_price`, `PatchInstallationBody.fire_settings`; 4.4.2, issue #95) | **omit — el mecanismo NO converge, y es deliberado** | Desde 4.4.2 HTTP expresa el tri-estado de verdad (`Option<Option<T>>` + `deserialize_double_option`), pero **JSON Schema no distingue «clave ausente» de «`null` presente»**, así que un cliente MCP no podría mandar la diferencia aunque la tool la aceptara: las tools conservan sus flags `clear_*` (`clear_purchase_price`, `clear_cap`, `clear_due_date`) — doctrina de la Fase 2, intacta. **Paridad de capacidad, no de forma**: las dos superficies pueden borrar, con sintaxis distinta. Evaluación de esta pasada: `update_asset` sigue construyendo el borrado en Rust (ahora `Some(None)` en vez de `Some(Value::Null)`, misma semántica, cero cambio de esquema) y `update_fire_settings` no expone borrado en absoluto porque mergea campo a campo sobre lo ya resuelto — su superficie no se toca. **n/a**, catálogo en 68 tools sin cambios | Que MCP gane una forma de expresar «ausente vs null» en su esquema, o que un tercer campo estrene el tri-estado sin flag `clear_*` hermano |
...
| 2 ✅ | `create_allocation_rule` + `delete_allocation_rule` | cores extraídas; cache **FULL**, con `impact`; `delete` exige **`confirm_token`** | El aviso de esta fila («the sink invariant lives spread across the handler») era el trabajo real: encapsularlo destapó **dos bugs vivos** (`futurefin-failure-archaeology` §2.20). Decisión propia de la superficie: **`create_allocation_rule` NO puede crear el sumidero** (`SinkPolicy::Forbidden`, aplicado **dentro de la core**, no en el esquema) — crearlo donde no había redirige todo el sobrante de golpe y **no se deshace por el mismo canal**. El `confirm_token` del delete tiene su propio motivo: recrear la regla no restaura su prioridad, y mientras tanto el sobrante se ha ido por otro sitio. **Hueco de dos pasos cerrado en la Fase 6**: `patch_allocation_rule_core` recibe también `SinkPolicy`, `update_allocation_rule` pasa `Forbidden`, y el edit que convertiría una regla en sumidero devuelve el mismo 400 `sink_creation_not_allowed` |
...
| `GET /v1/transactions/transfer-matches` | **Tool nueva** `suggest_transfer_matches` (**sin `view`**: own-user por construcción). Regla dura aplicada: **GET aparte, nunca `?dry_run` sobre el POST**. (La deriva «default 15» del doc-comment se corrigió en la Fase 7: hoy schema y core dicen 1–365, default 30.) |
...
| `POST /v1/allocation-rules` + `DELETE /v1/allocation-rules/{id}` (ya existían) | **Dos tools nuevas**, cierran §3.2 #2. FULL + `impact`; el delete exige `confirm_token`. `SinkPolicy::Forbidden` en la creación (ver §3.2 fila 2; el hueco de dos pasos vía `update` quedó cerrado en la Fase 6) |
