...
| `crypto_frozen_vectors.rs` | 5 (+2 `#[ignore]` generators) | **no database**: frozen vectors of the crypto stack, the evidence bar for an `aes-gcm`/`argon2`/`flate2` bump. Decrypts `tests/fixtures/frozen-vector-v12.ffbackup` — a binary built with aes-gcm 0.10.3 · argon2 0.5.3 · flate2 1.1.9 — through the exact path of `import::decode_request_blocking` (`FFBK` framing → Argon2id → AES-256-GCM with AAD → **gunzip** → `parse_payload(12)` → `migrate_to_current`), and verifies a golden PHC hash minted by the real registration path (`hash_password_blocking`) through the real login path (`verify_password_blocking`), plus a separate test that today's registration still produces the same `Argon2::default()` params. Both halves ship a negative control so the belt cannot pass vacuously. The gzip layer is deliberate: the plaintext is compressed *inside* the sealed envelope, so the same vector guards `flate2`. **Do not run the generators to fix a red** — a vector regenerated with the new crate proves only what was already proven |
