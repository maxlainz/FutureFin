...
| 2 | Same input, inflation 0% vs 3%: only `jubilacion_month_index` and the engine's internal FIRE-target trace may change (this experiment predates the API — `fire_target_series` was the wire name until 5.0.0 v2 retired it entirely; the underlying engine function `fire_target_at_month_index` is unaffected and still varies with inflation); `net_worth` series must be **bit-identical** | Zero series delta. If the NW series moves with inflation → someone re-broke the v1.2.0 model; stop and treat as regression |
