...
| 4 | ~~**Negative expected returns silently clamped to 0%**~~ — **CERRADO, la fila era falsa desde antes de este barrido (Fase 7, 2026-08-29)**. `monthly_multiplier` (`crates/engine/src/projection.rs`) clampa a factor 1 **solo con `p.is_zero()` exacto**, y a 0 solo cuando `annual_factor <= 0` (⟺ `p <= −100`, pérdida total). Un −5 %/año **compone hacia abajo con normalidad**, y hay test que lo fija: `negative_return_composes_downward` (−50 % ⇒ factor mensual 0,5^(1/12), que compuesto 12 veces reconstruye 0,5). `futurefin-fire-domain-reference` ya llevaba la corrección fechada 2026-08-28; nunca se propagó aquí | `crates/engine/src/projection.rs::monthly_multiplier` | [CBD] |
