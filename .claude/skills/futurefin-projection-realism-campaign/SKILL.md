...
| 8 | Planning flows («Próximos») without `due_date`: total spread evenly over **90 civil days** from ref_date; dated flows land in their calendar month, past-dated ones dropped | `PLANNING_UNDATED_SPREAD_DAYS=90`, `apps/api/src/handlers/projection.rs:298,331-384` | [AS] |
...
| 11 | Liabilities carry **no interest**: `monthly_payment` reduces principal 1:1 (capped at remaining principal) | `projection.rs:540-555` | [GAP] — debt amortizes far faster than a real loan; NW optimistic while debt is live |
...
| 11 | Liability 100.000 € principal, 500 €/mes payment | Engine: debt gone in exactly 200 months. Real 3%-interest loan at 500 €/mes: ≈ 430 months. Effect: **~19 years of debt service missing**; interim NW overstated by cumulative interest (~115.000 € over the real loan's life) |
...
- Liabilities still interest-free: read the principal-reduction block, `grep -n "principals\[i\] -= pay" crates/engine/src/projection.rs`.
