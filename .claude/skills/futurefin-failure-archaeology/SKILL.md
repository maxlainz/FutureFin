...
description: >
  ...
  parameter on the `get_projection` MCP tool, evaluating the withdrawal-rule failure F3
  (`RuleBelowNeed`) month by month instead of only in the first retired month, or renaming a field
  to disambiguate it from a same-named field elsewhere instead of declaring its basis/mode.
...
| 24 | **F3 (`RuleBelowNeed`) evaluada mes a mes con latch** — «con una regla por saldo, lo que la regla permite queda por debajo de la necesidad ordinaria», comprobado en CADA mes `Retired` | Era un problema de **BARRERA**, no una medición del plan: el permitido sigue a `L(k−1)`, que pasea con ~17 % de volatilidad frente a una deriva de ~0,8 %/año, así que sobre cientos de meses la probabilidad de tocarla alguna vez tiende a 1 por la varianza y no por la salud del plan. Bajo `rule_is_spend` una regla porcentual NO puede disparar F1 (una fracción del saldo nunca lo vacía), así que la barrera era el único veredicto posible y decidía sola. Medido sobre la demo sintética con «3,5 % del saldo»: capital necesario hoy **2,52 M€** frente a **620 k€** con `fixed_real`, **67 fallos de 2.500 caminos, todos F3**, y el primero siempre antes de la pensión (mediana: mes 293); el hogar de prueba de `mc_percent_of_balance_never_ruins_but_cuts_the_spending` fallaba en el 94,9 % de los caminos sin que la cartera llegara nunca a cero. Sustituida por **F3 solo en `R`**, el primer mes jubilado: con `R` fijo, F2 y F3 son la misma pregunta en dos unidades (bruta y neta) y después manda F1 mes a mes — el mismo hogar pide **860 k€**. **Retirarla del todo NO era la alternativa**: colapsaría al suelo de F2 y el modo porcentual sería infalible por construcción | Retirada **2026-09-07**, decisión C10 del owner (5.0.0, paquete E11); `crates/engine/src/sim_core.rs` (bloque «EL VEREDICTO DE ESTE CAMINO»), regresiones `crates/engine/tests/phases_wp3.rs::f3_fires_only_in_the_first_retired_month_never_after` y `crates/engine-stochastic/tests/monte_carlo.rs::mc_f3_is_a_property_of_the_plan_not_of_the_draw` |
...
| **5.0.0-rc UX de Jubilación, SEGUNDA vuelta** (U1b): el acordeón «Avanzado» con la línea «Supuestos: …» de cabecera, y el aviso de alta «Elige tu estrategia» descartable por navegador. El acordeón repartía **13 de 21 campos** en seis secciones plegadas —tres mandos de la pensión a dos pantallas de la casilla «Cuento con una pensión», la base del gasto parcial lejos de la media jornada—, y su línea «Supuestos» existía como contrapeso U12: si escondes un campo, enuncia su valor o lo estás forzando en silencio | Veredicto del owner sobre la demo (F10): **«Avanzado es un cajón de sastre mal explicado. Idealmente desaparece si la información se organiza y jerarquiza bien»**; y sobre el banner (F5): **«"Elige tu estrategia" es innecesario cuando ya hay una seleccionada»** — su descarte vivía en un flag de `localStorage` que **nunca miraba el perfil**, así que reaparecía en cada navegador nuevo por muy configurado que estuviera el plan. La línea «Supuestos» era la prueba de que el acordeón sobraba: hacía falta enunciar por fuera lo que el acordeón escondía por dentro | **Tarjetas por tema, todo a la vista** (V3): Estrategia · Edades · Pensión · Gasto en jubilación · Retirada · Horizonte, cada una con su frase de qué hace y qué implica cambiarla, y sin ninguna vacía. Sin acordeón no hay nada escondido que enunciar, así que la línea y su módulo (`lib/assumptions-line.ts`) se retiraron con él; el banner y `lib/retirement-intro.ts`, también. **Regla que queda**: un aviso de onboarding se apaga con el ESTADO que lo hace innecesario, no con un flag por navegador |
| **Tile «Objetivo al jubilarte» de Proyección alimentado por `jubilacion_target_net_worth`** (v1.3.0 → 5.0.0): el escalar es la base del objetivo evaluada en el mes 0 —inmóvil por contrato (`api-routes.md` §projection)— y la tarjeta lo rotulaba «al jubilarte» con la fecha del cruce al lado, sin depender del toggle «En dinero de hoy». Mientras el objetivo fue ~una perpetuidad, base(0) era buen proxy del objetivo en euros de hoy y nadie lo notó | Con el objetivo puente de 5.0.0 el objetivo DECRECE hasta 0 en el mes de la pensión: en la demo el tile decía 1.609.855 € frente a 1.148.467 € nominales del mes 243 = 696.563 € de hoy (**2,31×**). El owner lo vio como «la gráfica no cambia el objetivo en dinero de hoy» (F11); la línea del chart sí se deflactaba — el tile no estaba en el camino del toggle | ~~`jubilacionTargetTileValue` (`lib/projection-chart.ts`)~~ **RETIRADA con el modelo v2 (E4/A5, 2026-09-06)**: `jubilacion_target_net_worth`/`jubilacion_target_net_worth_nominal`/`fire_target_series` salieron del wire — no hay objetivo con puente que declarar en ninguna base. El tile de Proyección se sustituyó por «Capital necesario hoy» (`neededCapitalAtRetirement`, `lib/projection-chart.ts`), que lee la MISMA cifra que Jubilación y el Resumen (`needed_capital_today`, M9): una sola magnitud publicada una vez, no un escalar por pantalla que declarar caso a caso. **La regla que queda sigue siendo la misma**: una cifra de dinero no se publica sin su base declarada al lado (hoy: `amount_today` vs `amount_nominal` en `NeededCapital`) |
| **5.0.0-rc riesgo de Jubilación** (U1b): la tabla «Probabilidad de agotar el capital» por edad (una celda cada cinco años) junto al KPI de éxito, con la banda 10–90 % pintada de un azul plano sin significado; y un **umbral de éxito configurable** (50–99 %, de serie 95) contra el que se coloreaba el KPI | Veredicto del owner (F6/F7/F8): **«el gráfico de riesgo no deja nada claro qué representa cada cosa»**, **«los tiles de riesgo son inútiles: no muestran nada que la gráfica no muestre — si la gráfica estuviera bien hecha no harían falta»**, «añadir riesgo en color rojo/verde». Sobre el umbral: «lo lógico sería poner un umbral de éxito del 100 %» — un listón configurable no hacía el plan más seguro, solo más indulgente consigo mismo | **La tabla no vuelve** (sigue siendo cierto: el color + el hover dicen lo mismo con más resolución). **El corte FIJO al 100 % (V7) fue una parada intermedia, no el destino**: el modelo de jubilación v2 (C3, panel adversarial 2026-09-06) **devolvió el umbral configurable**, pero corrigiendo el motivo original de retirarlo — no como un listón arbitrario contra el estimador puntual, sino como una restricción evaluada sobre el **límite inferior del intervalo de Wilson** (`success_wilson_low`), con la fecha de jubilación DECIDIDA por ese umbral (no un veredicto posterior sobre una fecha ya fija). Medido antes de la corrección: con el estimador puntual y el umbral fijo al 100 %, la fecha era el **mínimo muestral** — bailaba ±10 años según la semilla, sin converger al subir N. Ver las cuatro filas de abajo, del mismo panel |
| **El objetivo (con o sin puente) como disparador de la fecha de jubilación** (todo 5.0.0 hasta E4, incluida la primera versión del «modelo v2»): un capital estático —perpetuidad o puente descontado— que el patrimonio determinista cruzaba, y ese cruce decidía cuándo se jubila el hogar | Medido en la demo antes del panel adversarial (2026-09-06): la fecha NO dependía del éxito — 42 años de edad con solo el 96,8 % de los escenarios sin agotar la cartera bajo «gasto fijo real», y un 100 % TAUTOLÓGICO bajo «% del saldo» mientras la mediana de los caminos pasaba **65 meses por debajo del gasto declarado** (la regla nunca puede arruinar a nadie, así que el veredicto binario no medía nada). Además, el objetivo con pensión caía a **0 €** el mismo mes en que empezaba a cobrarse, aunque el radio dijera «ignora la pensión» — un acantilado de construcción, no un bug aislado | **La fecha la decide el ÉXITO** (M1, definición A): primer mes `k` tal que, jubilándose en `k`, una fracción ≥ umbral de miles de caminos sorteados no fallan nunca hasta el horizonte (`crates/engine-stochastic::solve_mc::valid_retirement_month`). El objetivo sobrevive SOLO como lectura informativa (el «número FIRE clásico», §2.4 de `financial-contracts.md`) y no dispara nada, en ninguna estrategia (D24 de `architecture-contract`, reescrita para el modelo v2) |
| **El colchón de caja derivado del tope de una regla de ahorro propia** (5.0.0 V6, decisión del owner de esa fecha): sin input que pedir, el motor calculaba un colchón «hasta X €» a partir del tope de la regla de ahorro que apuntaba al líquido sin volatilidad, y lo mantenía relleno con un mecanismo de Monte Carlo aparte | Medido por el panel adversarial: la derivación producía un colchón que saltaba **de 12 a 19 a 154 meses** de gasto según en qué otra base se leyera el mismo tope — una cifra que no describía ninguna decisión del usuario, solo un artefacto de qué regla de ahorro tenía tope ese día. El propio modelo v2 (antes de la corrección) ya había medido que el colchón COSTABA rentabilidad neta más de lo que protegía en algunos escenarios (`futurefin-fire-domain-reference` §4b, «el colchón cuesta neto») | **El mecanismo se retira ENTERO** (M6): la caja es un activo más: la regla de ahorro que el usuario configura fija cuánto se guarda, sin una segunda cifra derivada que pudiera desincronizarse de la primera. Cero tipos, cero campos, cero tests en `crates/engine` ni en `crates/engine-stochastic` — verificar con `grep -rnE "CashBuffer\|cash_buffer" crates/engine/src crates/engine-stochastic/src` (vacío) |
| **F2 (tasa inicial excesiva) como comprobación MENSUAL sobre el saldo vivo** (primera versión del modelo v2, ANTES del panel adversarial del 2026-09-06 por la mañana): cada mes jubilado, comparar la venta ordinaria del mes contra `SWR/12 · L(k−1)` y fallar el camino si se excedía, en vez de comprobarlo solo el primer mes | El panel adversarial reimplementó el modelo v2 en un Monte Carlo independiente con los datos de la demo y encontró que esta definición ponía la fecha válida en la edad de la **pensión (72 años)** y el capital necesario en **56 M€** — porque CUALQUIER año malo del sorteo convertía un plan sano en un fallo retroactivo, así que el umbral solo se cumplía cuando el hogar ya no dependía de vender nada (la pensión cubría todo) | **F2 es la tasa INICIAL, solo en el primer mes jubilado `R`** (corrección C1): `12 · necesidad_ordinaria(R) > tope/100 · L(R−1)` ⇒ falla en `R`, una comparación que ocurre UNA vez por camino, exactamente como Bengen/Trinity anclan la tasa de retirada al capital de partida (`InitialRateGate`, `crates/engine/src/phases.rs`) |
| **La curva del gráfico de «capital necesario» escalada por mediana/cuantil** (`needed_c(k) · mediana(k) / q(k)`, primera versión del modelo v2, decisión M9 original): en vez de resolver el capital necesario en cada punto de la rejilla, escalar el capital de HOY por la razón entre la línea determinista y algún cuantil de los caminos sorteados en `k` | El panel adversarial midió que la curva así construida era **algebraicamente incapaz de cruzar la línea de patrimonio en la fecha válida** (el punto que por definición SÍ tiene que estar sobre el umbral): con el hogar de la demo la curva salía casi PLANA (4–5 M€ en vez de crecer con el gasto indexado) y el nodo del horizonte quedaba AUSENTE, porque el cociente se rompía justo donde el hogar real se agotaba | **Bisección real en cada nodo** (corrección C4): `needed_capital_curve` resuelve, para cada edad de la rejilla, el factor `λ` que escala el patrimonio LÍQUIDO (valor y base de coste) hasta que el sorteo cumple el umbral — la misma pregunta que el capital de hoy, contestada mes a mes (`crates/engine-stochastic::needed_capital::needed_capital_curve`). La curva NO tiene por qué cruzar la línea de patrimonio: lo que la cruza es el escenario que el umbral obliga a salvar, marcado por la marca vertical de la fecha válida, no por un cruce de curvas |
...
| Resucitar `jubilacion_target_net_worth`/`fire_target_series`, o publicar un importe sin decir en qué euros está | §3, fila del tile de Proyección — los dos campos salieron del wire con el modelo v2 (E4/A5); la sustituta es «Capital necesario hoy» (`needed_capital_today`, la MISMA cifra en Jubilación/Resumen/Proyección), y la regla de declarar la base sigue viva en `NeededCapital::{amount_today, amount_nominal}` |
| Volver a meter la configuración de Jubilación en un acordeón «Avanzado» (o en cualquier plegado que esconda supuestos «para simplificar») | §3, fila del acordeón U1b — la línea «Supuestos» que hacía falta por fuera ERA la prueba de que el acordeón sobraba. Si un campo mueve tus números, va a la vista y con la frase que dice qué mueve |
| Volver a TABULAR el agotamiento por edad, o añadir un tile que repita lo que el gráfico ya dice | §3, fila del riesgo — la pregunta que hay que contestar primero es qué dice esa tabla que el color de la banda y su hover no digan ya |
| Volver a un veredicto con corte FIJO (sin umbral), o dejar que el CLIENTE elija el listón que colorea el semáforo | §3, fila del riesgo — el corte SIEMPRE lo decide el SERVIDOR (`success_verdict`); desde el modelo v2 (C3) el umbral SÍ es configurable (80–100, perfil del usuario) pero se evalúa contra la cota inferior de Wilson, no contra el estimador puntual — el problema nunca fue que el umbral se pudiera ajustar, fue que un estimador puntual sin intervalo hacía la fecha inestable con la semilla |
| Apagar un aviso de onboarding con un flag de `localStorage` | §3, fila del acordeón U1b — el banner de alta de Jubilación reaparecía en cada navegador nuevo por muy configurado que estuviera el plan. Se apaga con el ESTADO que lo hace innecesario |
...
| Pintar resultados/tiles por ENCIMA del formulario que los produce, apilar tarjetas de resultado sin jerarquía entre ellas, o dar a cada chart su propio eje/ventana | §3 fila 6 — orden fijado por U1/U5/U7 y revisado en la tercera vuelta (V3): config en tarjetas por tema, todo a la vista → resultado, como mucho 3 tiles con una frase-hito, un solo chart |
...
**Ampliado 2026-09-06 para el modelo de jubilación v2** (WP D2, panel adversarial 2026-09-06,
rama `release/5.0.0`): §3 gana cuatro filas de diseños rechazados por el panel (objetivo como
disparador, colchón derivado del tope, F2 mensual sobre el saldo vivo, curva de capital necesario
escalada por mediana/cuantil) y corrige el «Replacement (current)» de dos filas viejas que la
tercera vuelta de UX había dejado escritas como si fueran el estado final: la fila del tile
«Objetivo al jubilarte» (`jubilacionTargetTileValue` ya no existe) y la fila del riesgo de U1b (el
corte fijo al 100 % de V7 fue una parada intermedia, no el destino — C3 devolvió el umbral
configurable, evaluado sobre Wilson). Re-verificación: `grep -n "jubilacionTargetTileValue" apps/web/src/lib/projection-chart.ts`
(vacío — retirada) y `grep -n "fn success_verdict" -A 10 apps/api/src/handlers/projection_bands.rs`
(compara contra `threshold_pct`, no un literal `100`).
...
**§3 filas 7–8 y sus cuatro filas de §4 se añadieron el 2026-09-05**, tras la TERCERA vuelta de UX de
Jubilación (V1–V7; feedback F2 y F5–F10 del owner sobre la demo local de `release/5.0.0`, mismo
issue #207). Documentan como formas RECHAZADAS el acordeón «Avanzado» + su línea «Supuestos», el
banner de alta con flag de `localStorage`, la tabla de agotamiento por edad y el umbral de éxito
configurable. Re-verificar:
- Los dos módulos retirados ya no existen: `ls apps/web/src/lib/assumptions-line.ts apps/web/src/lib/retirement-intro.ts` (los dos FALLAN)
- El acordeón no tiene ni regla ni consumidor: `grep -c "^\.retirement-advanced" apps/web/src/App.css` (**0**) y `grep -c "^export const ADVANCED_SECTION_LABEL" apps/web/src/lib/retirement-form.ts` (**0**)
- Las tarjetas que lo sustituyen: `grep -n "PLAN_CARD_ORDER" -A 8 apps/web/src/lib/plan-fields.ts` (seis ids)
- La tabla de agotamiento no tiene ni CSS ni consumidor: `grep -c "risk-depletion" apps/web/src/App.css apps/web/src/views/RetirementView.tsx` (**0** en los dos)
- El color de la banda vive en un módulo puro con sus cortes absolutos: `grep -n "RISK_AMBER_AT\|RISK_RED_AT" apps/web/src/lib/risk-gradient.ts`
- El umbral desapareció del perfil de cliente: `grep -c "success_threshold_pct" apps/web/src/lib/retirementProfile.ts` (**0**)
